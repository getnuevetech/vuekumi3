...
| Partner/API distribution | **Yes** (Phase 33) — read-only `GET /api/partner/v1/photos[/:id]` behind admin-issued bearer keys (hash-stored, shown once, revocable, 120 req/min per key). Serves **cleared inventory only** (active + stock permission states; never private/portfolio/agency-protected), with attribution and licence flags computed by the same guards as checkout. Licences are granted on VueKumi, not by the API. Terms **forbid AI training** through this API even when a photograph has a Phase 34 opt-in. Dataset pricing is undecided. |
| Separate AI-training consent | **Yes** (Phase 34) — separate opt-in; dataset pricing undecided; not sold; partner API still forbids training |
| Featured homepage slots | **Yes** (Phase 41) — staff pin live stock photographs into hero / edge / editorial / pricing / stats background at `/admin/homepage`. Unfilled positions keep the live ranking fallback. Featuring is not a licence and not AI-training consent. Private, portfolio, and agency-protected inventory cannot be pinned. |
...
Phases 35–40 are shipped. Arc D is complete. Phase 34 (AI-training opt-in) is shipped as a consent engine; dataset pricing remains undecided and VueKumi does not sell training access. Phase 41 ships admin-curated homepage featured slots. Models may public-register and upload; VueKumi contacts named photographers. `AdminRole` is a capability preset (Phase 36). Country overlays are product notices (counsel-gated copy). Consent withdrawal locks new sales and does not silently void past grants.
