...
Phases 35–40 are shipped. Arc D is complete. Phase 34 (AI-training) stays parked. Models may public-register and upload; VueKumi contacts named photographers. `AdminRole` is a capability preset (Phase 36). Country overlays are product notices (counsel-gated copy). Consent withdrawal locks new sales and does not silently void past grants.
