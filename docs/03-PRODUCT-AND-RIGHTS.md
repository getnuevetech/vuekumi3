...
Phases 35–40 are shipped. Arc D is complete. Phase 34 (AI-training opt-in) is shipped as a consent engine; dataset pricing remains undecided and VueKumi does not sell training access. Models may public-register and upload; VueKumi contacts named photographers. `AdminRole` is a capability preset (Phase 36). Country overlays are product notices (counsel-gated copy). Consent withdrawal locks new sales and does not silently void past grants.
